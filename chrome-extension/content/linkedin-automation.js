// LinkedIn automation content script (D-05: OFF by default) — RETAINED, NOT DELETED.
//
// This file does nothing on its own: it only answers messages from the service worker (EXECUTE_ACTION,
// CHECK_STATUS_AND_SCRAPE), which the worker only sends while LinkedIn automation is allowed (server flag on + `automation`
// scope + the optional `debugger` permission granted). It also refuses locally if the stored session says automation is off.
//
// The lead-capture card that used to live here (the "Add to Lead List" widget) is now content/widget.mjs.
//
// Not declared in the manifest: automation/inject.js injects it on demand, only while automation is allowed.
window.__lgAutomationLoaded = true;

// content.js
console.log("LeadGennie Agent: Content Script Initialized");

const LINKEDIN_DEBUG = false;
...
// Defence in depth: even if something did message this script, it refuses unless the stored session allows automation.
async function automationPermitted() {
  const { session } = await chrome.storage.local.get('session');
  return Boolean(session && session.features && session.features.linkedinAutomation && (session.scopes || []).includes('automation'));
}

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'CHECK_STATUS_AND_SCRAPE' || request.type === 'EXECUTE_ACTION') {
    automationPermitted().then((ok) => {
      if (ok) return handleAutomationMessage(request, sendResponse);
      sendResponse({ success: false, error: 'LinkedIn automation is turned off.' });
    });
    return true;
  }
});

function handleAutomationMessage(request, sendResponse) {
