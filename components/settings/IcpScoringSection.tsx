...
const inputCls =
  "w-full rounded-lg bg-neutral-50 border border-neutral-200 px-3 py-2 text-sm text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-300 disabled:opacity-60";
...
export default function IcpScoringSection({ value, onChange, canEdit }: { value: ScoringDraft; onChange: (v: ScoringDraft) => void; canEdit: boolean }) {
  return (
    <section id="scoring" className="space-y-4 scroll-mt-20">
      <div>
        <h2 className="text-base font-bold text-neutral-900">Scoring</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {WEIGHT_LABELS.map(([key, label]) => (
          <label key={key} className="block">
            <span className="block text-sm font-medium text-neutral-700 mb-1.5">{label} weight</span>
            <input
      ...
      <label className="block">
        <span className="block text-sm font-medium text-neutral-700 mb-1.5">Keywords that signal fit</span>
        <input
        ...
        />
        <span className="block text-xs text-neutral-500 mt-1">Matched against verified evidence (a company&apos;s own pages, job listings, news). Each adds to the score if found.</span>
      </label>
      <label className="block max-w-[200px]">
        <span className="block text-sm font-medium text-neutral-700 mb-1.5">Qualified at score ≥</span>
        <input
          type="number" min={0} max={100} inputMode="numeric" disabled={!canEdit} className={inputCls}
