...
export default function ToneSetting({ initial, canEdit }: { initial: Tone; canEdit: boolean }) {
  ...
  return (
    <section className="space-y-4" aria-labelledby="tone-heading">
      <div>
        <h2 id="tone-heading" className="text-base font-bold text-neutral-900">Email tone</h2>
        ...
        {TONES.map((t) => (
          <button
            ...
            className={cn(
              ...
              tone === t ? "border-emerald-300 bg-emerald-50" : "border-neutral-200 bg-white hover:bg-neutral-50 hover:border-neutral-300"
            )}
          >
            <span className="block text-sm font-medium capitalize text-neutral-900">{t}</span>
      ...
      <div className="h-5 text-sm" role="status">
        ...
        {message && <span className={message.ok ? "inline-flex items-center gap-1.5 text-emerald-600" : "text-rose-600"}>{message.ok && <CheckCircle2 className="h-4 w-4" />}{message.text}</span>}
