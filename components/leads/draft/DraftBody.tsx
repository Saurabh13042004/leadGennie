        ...
        return (
          <span key={i} className="group relative">
            <a
              ...
              className="rounded bg-emerald-400/15 px-0.5 text-emerald-100 underline decoration-emerald-400/50 decoration-dotted underline-offset-2 hover:bg-emerald-400/25 focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
              aria-label={`Backed by ${ev.sourceTitle ?? ev.sourceUrl}. ${ev.snippet}`}
            >
              {seg.text}
            </a>
            <span role="tooltip" className="pointer-events-none absolute left-0 top-full z-20 mt-1 hidden w-80 max-w-[80vw] rounded-lg border border-white/15 bg-[#111] p-3 text-xs shadow-xl group-focus-within:block group-hover:block">
              <span className="block font-medium text-emerald-200">Verified evidence</span>
              <span className="mt-1 block text-neutral-300">“{ev.snippet.length > 220 ? `${ev.snippet.slice(0, 220)}…` : ev.snippet}”</span>
              <span className="mt-2 flex items-center gap-1 text-neutral-500">
