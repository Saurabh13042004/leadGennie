...
- **Search provider (D-03, decided 2026-09-26):** `SEARCH_PROVIDER=tavily` (`TAVILY_API_KEY`) — `TavilySearch` does web (`topic=general`) and news (`topic=news`, last year) from one key, `basic` depth = 1 credit; free plan 1,000 credits/month with no card. `brave` remains available (`BraveSearch`; new accounts need a card). Unset ⇒ `NullSearch`: connectors skip with a visible warning (`web_search_unavailable`, `news_unavailable`) and the run continues on first-party pages + job boards.
- **News fallback:** `NEWS_FALLBACK=gdelt` wraps the provider in `ChainedSearch`: news that fails on the primary (quota, rate limit, outage, bad key) is answered by `GdeltNews` (GDELT DOC 2.0, free, no key, ~3-month window, title + date only, no snippet) and every switch is a run warning (`news_fallback:<provider>:<code>`). With `SEARCH_PROVIDER=none` it is news-only. Per-run circuit breakers: a primary that failed for good (quota/auth) is skipped, and a failed fallback is not called again, for the rest of that run. **GDELT is best-effort:** measured live 2026-09-26 it took ~14 s for a trivial query and then answered every further request with `429` (16–20 s each) even 12–27 s apart, so the adapter serializes/spaces calls (5.5 s), uses a 30 s timeout and treats its plain-text 429/"too short" replies as rate-limit/empty — never as an error that fails the run. Don't rely on it for coverage.
- **Rejected:** Gemini *Grounding with Google Search* — free (1,500 grounded requests/day on 2.5 models, 5,000/month on 3.x) but the Gemini API terms bar caching/analyzing/learning from grounded results and require displaying Search Suggestions to the end user; Google Custom Search JSON API — closed to new customers, ends 2027-01-01.
- **Tavily was not exercised live** (no key yet): the adapter is covered by mocked-HTTP tests of the documented request/response shapes and error codes (401/403/429/432/433/5xx); confirm with the first `make smoke` once `TAVILY_API_KEY` is set.
