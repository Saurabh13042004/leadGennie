...
## As built (mailbox OAuth, D-04) — migration `0014`

Additive; every existing row is a `resend` mailbox and keeps its meaning.
- **`mailboxes`:** `domain_id` now nullable (`CHECK (provider <> 'resend' OR domain_id IS NOT NULL)`); `provider` ∈ `resend|gmail|microsoft`; `status` ∈ `pending_approval|active|paused|reconnect_required|disconnected|error` (`active` = "connected"). New: `owner_user_id`, `display_name`, `provider_account_id` (unique per workspace+provider), `access_token_enc`/`refresh_token_enc` (AES-256-GCM ciphertext, read only by `lib/domain/mailboxes/repository.loadSecrets`), `token_expires_at`, `token_version` (compare-and-swap on refresh), `scopes text[]`, `last_synced_at`, `last_error`, `connected_at`, `disconnected_at`, `updated_at`. **No `sent_today` column** — it is counted from `messages`, the rows the send gate counts.
- **`messages`:** `provider_thread_id` (Gmail `threadId` / Graph `conversationId`, indexed per workspace), `dispatched_at` (write-ahead marker for providers without an idempotency key — see `mailboxes.md` §2).

## As built (Phase 5) — migration `0012`
- **`messages`:** the durable record of every outbound email attempt. One per scheduled send (`unique(campaign_send_id)`), **inserted before the provider is called**. Stores the exact `subject`/`body`/`headers` handed to the provider (a retry re-sends this, never a re-render), `idempotency_key`, `provider_message_id` (unique per provider), status `sending|sent|delivered|bounced|complained|failed|canceled`, `error`/`error_class`, `attempts`, `soft_bounce_count`, `claimed_at` (= when it was counted against limits/spacing) and per-event timestamps. Indexed for the gate's count queries (campaign, mailbox, workspace, recipient domain × `claimed_at`). Sent legacy sends are backfilled (`idempotency_key = 'legacy:'||id`). `direction 'in'` is reserved for Phase 6.
- **`message_events`:** append-only provider events; `unique(provider, provider_event_id)` makes webhook redelivery a no-op.
- **`workspaces.sender_name`, `sender_address`:** the CAN-SPAM/GDPR footer identity; launch is blocked without both (builder campaigns). **`workspaces.daily_send_cap`:** optional workspace-wide ceiling (no UI yet).
- **`campaigns.paused_reason`:** why the system paused a campaign (bad key/domain, outage, no provider key); cleared on resume.
- **`jobs_send_payload_idx`:** `((payload->>'campaignSendId')) where type='campaign_send'` for the scheduler's "does this send already have a live job?" lookup.
- Written only by `lib/domain/sending/*` (single writer). Both new tables are in the tenancy gate.
