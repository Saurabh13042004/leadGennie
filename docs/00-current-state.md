...
| Email | **Mailbox OAuth (D-04, migration 0014):** users connect Gmail/Google Workspace or Microsoft 365 by signing in (`/api/mailboxes/[provider]/connect|callback`); a per-mailbox provider registry (`lib/domain/mailboxes/registry.ts`) resolves `GmailMailboxProvider` / `MicrosoftMailboxProvider` / `ResendProvider`; tokens AES-256-GCM at rest with compare-and-swap refresh; providers with no idempotency key are protected by a write-ahead `messages.dispatched_at` — see [`mailboxes.md`](mailboxes.md). **Phase 5:** `MailProvider` port (`lib/email/provider.ts`) with `ResendProvider` + `FakeMailProvider`; sending is the `campaign_send` job (`lib/domain/sending/*`), durable `messages` + `message_events`; webhook records delivered/opened/clicked/bounced/complained idempotently. Domains via `resend-domains.ts` |
