...
**Decision (search/news provider, 2026-09-26):** _decided_ — **Tavily** (`SEARCH_PROVIDER=tavily`; free plan 1,000 credits/month, no card; web + news from one key) with an opt-in **GDELT** keyless news fallback (`NEWS_FALLBACK=gdelt`). Rejected: Brave (free plan removed for new accounts, card required), Google Custom Search (closed to new customers, shuts down 2027-01-01) and **Gemini "Grounding with Google Search"** — free, but its terms forbid caching/analyzing grounded results and require showing Search Suggestions to the end user, which an automated evidence pipeline cannot honor. Adapter and a key are the only things missing for a live run. The people/company provider (part 2, Phase 8) is still _pending_.
