...
## Field-report fixes — campaigns that "sent nothing", stale lead counts, ICP score, ICP keywords (2026-09-26)

Reported after connecting Gmail, launching a campaign and deleting leads. Investigated read-only against the live DB before changing anything.

- **"Campaign ran, no emails went out" — nothing was broken in the send path; three things combined.** (1) Launched Friday 21:12 UTC into a UTC 09–17 Mon–Fri window, so the first send was Monday 09:00. (2) The user paused and resumed 9 seconds later and `resumeCampaign` shifted every pending send by `ceil(pause length)` = **one whole day** even though nothing had come due — Monday became Tuesday (bug, fixed: it now shifts only when a send actually became due while paused; regression test). (3) No worker process was running (no `campaign_send` job since the day before; `npm run dev` does not start one — use `npm run worker` or `npm run dev:all`). Also: new campaigns now default the send-window timezone to the browser's (was UTC — a 9–5 window in the middle of the night for most people), and the campaign page says "Nothing has gone out yet — the first email is scheduled for …" instead of just showing zeros.
- **Deleting a lead didn't reach campaigns/audiences.** (a) `campaigns.total_leads`/`blocked_count` are stored at launch and were never adjusted; (b) the campaign builder's audience picker and the edit page read the stored `segments.lead_count` (only the Audience page measured live); (c) any lead with even a *pending* send was refused as "has message history". Now: a lead that was never emailed (only pending/canceled/blocked sends, no `messages`) can be deleted and the campaigns' counters drop in the same statement; pickers use `measureSegment` (live); delete revalidates campaigns and lead lists. Leads that were actually emailed are still kept (Do Not Contact instead).
- **ICP 15/100 for a CTO at a startup.** The 15 was exactly the title criterion's share: eight keywords at weight 10 each = 80 of 165 total weight, all "unknown" without evidence (and the engine wasn't configured in that dev server — `INTELLIGENCE_*` set after it started; restart it and run the engine). Keyword signals now share a fixed budget (`KEYWORD_WEIGHT_BUDGET` = 20 total, relative weights kept; done in `toEngineIcp`, so existing saved ICPs improve and "Re-score" applies it). A low-confidence score is no longer labelled "Below threshold" — it says "Needs more evidence". Scoring semantics are unchanged (unknown still counts 0; docs/intelligence-engine/scoring.md).
- **"Fill with AI" for Keywords that signal fit** on Settings → Positioning: `suggestIcpKeywords` (admin, zod-validated, usage-metered, `FakeLlm`-testable) proposes company-level signals from what you sell; role words and target titles are dropped in code. Read-only — nothing is saved until Save changes.
- Not verified: pages were not loaded in a browser (built app compiles; component render tests + 1,140 tests pass); no live OpenAI call was made for the suggester.
...
**Follow-up 2026-09-26 — company names missing on real LinkedIn profiles, and email suggestions (owner request)**
- *Why the company was blank:* real profiles are titled just `Name | LinkedIn`, so the company must come from the page text or the model. My first version only accepted a model answer if it appeared **verbatim** on the page (so "Acme Inc" was dropped for "Acme, Inc."), ran the model only in some cases, and hid failures behind "unavailable". Now: free readers first (`Current company:` label, the headline "Title at Company", the first Experience entry — never About prose), then the **model reads the page** for whatever is still missing, with a format-forgiving but content-strict grounding check (case, accents, punctuation, "Inc/LLC" ignored; words must still be on the page), and the card shows the **real reason** when the model fails (quota, bad key).
- *Email suggestions:* `POST /api/extension/capture/suggest` builds candidates from name + company website (`first.last`, `first`, `flast`, `firstlast`, `f.last`, …), ranks first the format your **own** leads at that domain already use (with "matches N emails you have"), and returns nothing for a domain with no mail server (real MX lookup; an inconclusive lookup doesn't hide them) or a personal mail provider. With no website yet it offers website guesses that at least accept mail. The card labels everything **"Auto-generated · may be wrong"**, never fills it in by itself, warns when one is chosen and again after saving. A chosen guess is stored as an ordinary `unverified` email with **low-confidence provenance (0.2)** and `email_guessed` in the activity — never as a verified fact.
- Tests: +26 (patterns/accents/transliteration, format inference, LinkedIn text reading incl. "never mistake About prose for the headline", grounding, suggest route incl. isolation/scope/MX, provenance flag, model-failure messages) and 2 new real-Chrome steps; **22/22** in Chrome, 900+ tests.
- Bugs found while doing it: About prose was being read as a headline (`I am a Staff Engineer at Acme`); letters like ł/ø/ß were dropped from guessed addresses (`ukasz@`).
- Not verified: a live model call (costs real money; the schema/prompt path is unit-tested and the failure path is exercised in Chrome with a deliberately invalid key), and a manual pass on real LinkedIn.
